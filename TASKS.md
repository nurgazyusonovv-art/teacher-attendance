...
- [x] Бардык домендер `admin_core`го көчтү: teachers, devices, attendance (dashboard, тарых, кол менен оңдоо, сабак кечигүүлөрү), schedules, school settings, QR, leaves. Пакет 1 327 сап; эки тиркемедеги repository катмары 662+787 → 482+29 сапка түштү жана эми жөн гана адаптер.
- [x] **Web'деги кош префикс багы жоюлду.** Бир катар web repo'лору жолду `${AppConstants.apiBaseUrl}` менен префикстеп, Dio'нун `baseUrl`ы да ошол эле маанини алып жүрчү. Vercel билдинде `--dart-define` абсолюттук URL бергендиктен иштеп калчу, ал эми Docker/nginx билдинде (dart-define жок) `/api/v1/api/v1/...` болуп сынмак. Эми бардык жол салыштырмалуу, `baseUrl` гана префиксти алып жүрөт — эки деплой тең иштейт.
- [x] **Telegram «тууралган эмес» багы табылды.** Эки жерде код `telegram_bot_token`ду жооптон окучу, бирок API аны эч качан кайтарбайт (write-only). Ошондуктан `isTgConfigured` **дайыма false** болчу — Telegram тууралса да «тууралган эмес» деп көрүнчү. Жалпы моделде андай талаа жок болгондуктан бул компиляция катасына айланып, ачыкка чыкты. Эми `telegram_enabled` + chat id боюнча аныкталат, ал эми токен талаасы бош башталат.
- [x] `LeaveStatus` жана `DeviceStatus` enum'дары: абалдын аталышы бир жерден келет, эки панель бирдей айтат. Белгисиз маани `pending` болот, `approved` эмес.
- [x] `WorkSchedule.startTime` nullable кылынды — web'дин эски модели аны `required String` кылып, дем алыш күнү жооп келсе кулай турган.
- [x] `devices` домени `admin_core`го көчтү: `TeacherDevice` + `DeviceStatus` enum (мурда эки жерде эркин сап эле) жана `DevicesRepository`. Абалдын аталышы (`Ырасталган`/`Күтүүдө`/`Жокко чыгарылган`) эми бир жерден келет, эки панель бирдей айтат.
- [x] **Mobile'га түзмөк ырастоо экраны кошулду** (`/admin/devices`, админ панелинин жогорку тилкесинен). Ансыз device binding'ди күйгүзүү мүмкүн эмес эле: админ телефондон иштейт, ал эми ырастоо web'де гана бар болчу. Экран жалпы `admin_core` repository'синин үстүндө курулду — код эки жолу жазылган жок.
- [x] Жокко чыгаруу ырастоо диалогун сурайт; күтүүдөгүлөр тизменин башында; ката учурунда кайра аракет кылуу бар. 6 widget тест, анын ичинде 320px экран.
- [x] `DeviceStatus.parse` белгисиз маанини `pending` кылат — `approved` эмес. Клиент түшүнбөгөн абал каттоого уруксат катары окулбашы керек (тест менен бекитилди).
...
### Башкы бет кайра жасалды — 2026-09-20
- [x] Макет боюнча: ак башкы тилке (аты, мектеби, байланыш абалы), чоң саат, келүү/кетүү жуп карточкасы, кара QR карточкасы, акыркы күндөрдүн тарыхы.
- [x] **Чоң саат телефондун сааты эмес.** `TodayStatusResponse.server_time` кошулду, клиент `SchoolClock` аркылуу ошол жерден тик-тик кылат. Телефону 10 мүнөт артта калган мугалим өзүн убагында деп ойлоп калбайт (AGENTS.md §5). Сервер жөнөтпөсө `--:--` көрсөтүлөт — телефондун сааты колдонулбайт. Түзмөктүн сааты артка секирсе көрсөтүлгөн убакыт артка кетпейт.
- [x] Уруксат суроо кнопкасы QR карточкасынын дал астына коюлду — мугалим бул жакка экинчи кезекте ушул үчүн келет, жана дал скандай албаган күнү ошол кнопка керек болот.
- [x] Тарых «Баары» шилтемеси толук тарыхка алып барат; башкы бетте акыркы 4 күн.
- [x] 15 жаңы тест (7 `SchoolClock`, 8 экран): сервердин сааты, саат жок учуру, кечигүү, кэштелген күндү скандоого болбошу, 320px экран.
- [x] Жол-жолой оңдолгон эки layout багы: `Row(stretch)` `ListView` ичинде чексиз бийиктик сурап жаткан, жана чоң текст масштабында дата сабы оң жакка ашып кетчү.
- [x] Backend 123 → 124 тест, mobile 68 → 83.
- [x] Версия 1.2.0+6 → **1.2.1+7**, Android билди кайра курулду (`releases/teacher-1.2.1-build7/`): ARM64 26MB (versionCode 2007), ARMv7 22MB (1007). Сертификат мурдагыдай (`91741bee…`), v2 схема, debuggable жок. 1.2.0+6 артефакттары өчүрүлдү — эч качан таркатылган эмес жана бирдей versionCode менен эки башка мазмун чаташтырмак.
...
- [x] Android релиз билди: `releases/teacher-1.2.0-build6/` — ARM64 26MB (versionCode 2006), ARMv7 22MB (1006). Экөө тең apksigner'ден өттү.
  - Кол коюу сертификаты мурунку релиз менен **бирдей** (`91741bee…`) — колдонуучулар үстүнө жаңырта алат.
...
- [x] App Review demo чечими — (а) варианты: изоляцияланган review tenant.
  - `schools.is_review_demo` желекчеси + `b8c04fa2e008` migration. Бар `DEMO-001` мектеби ошол migration'да белгиленип, радиусу дүйнөлүк (20 100 км) жана max accuracy 100 км кылып коюлду. Production мектеби тийилген жок (80 м бойдон).
  - **Geofence кодуна бир да шарт кошулган жок** — коопсуздук жолу бардык мектеп үчүн бирдей, бир гана ошол саптын радиусу кенен.
  - Инвариант: `is_review_demo` мектепке demo эмес аккаунт каттоо жасай албайт → `REVIEW_SCHOOL_FORBIDDEN` (403). Ошондуктан кенен радиус эч качан чыныгы мугалимге жол ачпайт.
  - `is_review_demo` `SchoolUpdate`та жок — API аркылуу эч бир мектепти «ачып» коюуга болбойт (тест менен бекитилди).
  - Demo желекчеси өзү эч нерсе бербейт: кадимки мектептеги `is_demo` аккаунт радиустан тышкары дагы эле `LOCATION_OUTSIDE_SCHOOL` алат.
