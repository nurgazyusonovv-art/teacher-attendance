...
class AbsenceService:
    @staticmethod
    async def catch_up(db: AsyncSession, school_id: str) -> None:
        ...
        reset = (await db.execute(select(AuditLog.created_at).where(
            AuditLog.school_id == school_id, AuditLog.action == "ATTENDANCE_RESET"
        ).order_by(AuditLog.created_at.desc()).limit(1))).scalar_one_or_none()
        from app.core.timezone import to_school_timezone
        start = max(date(2026, 9, 7), to_school_timezone(reset, school.timezone).date()) if reset else date(2026, 9, 7)
        if end >= start:
            await AbsenceService.process_workdays_from(db, school_id, start, min(end, start + timedelta(days=366)))
    ...
    async def process_workdays_from(
        ...
        if end_date < start_date or (end_date - start_date).days > 366:
            raise AppException(code=ErrorCode.VALIDATION_ERROR, message="Күндөрдүн аралыгы туура эмес.", status_code=400)
        total = 0
        current = start_date
        while current <= end_date:
            total += len(await AbsenceService.process_daily_absences(db, school_id, current))
    ...
    async def process_daily_absences(
        ...
        if eval_date < date(2026, 9, 7) or eval_date > now.date():
            return []
